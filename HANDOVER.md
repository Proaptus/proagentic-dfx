...
```
proswarm-template/
├── CLAUDE.md                 # Main Claude Code config (READ THIS)
├── HANDOVER.md               # This file
├── pytest.ini                # Test configuration
├── .gemini/GEMINI.md         # Cross-platform Gemini support
├── tests/
│   ├── unit/                 # 18 unit tests for hook logic
│   ├── integration/          # 10 integration tests
│   └── e2e/                  # 5 E2E workflow tests
└── .claude/
    ├── settings.json         # Hook configuration (relative paths)
...
| File | Change |
|------|--------|
| `.claude/settings.json` | Absolute paths → Relative paths |
| `.claude/hooks/enforce-proswarm.py` | Hardcoded path → Dynamic `__file__` resolution |
| `.claude/hooks/stop_validator_wrapper.sh` | Hardcoded path → `SCRIPT_DIR` variable |
| `CLAUDE.md` | ProAgentic-specific → Generic template |
