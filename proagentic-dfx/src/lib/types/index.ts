...
export interface ParetoDesign {
  ...
  trade_off_category?: 'lightest' | 'cheapest' | 'recommended' | 'most_reliable' | 'balanced' | 'conservative' | 'max_margin';
...
// Individual layer definition from the layup
...
// Thickness distribution contour point
export interface ThicknessContourPoint {
  r: number;
  z: number;
  t: number;
}
...
export interface DesignGeometry {
  ...
  thickness_distribution: {
    ...
    contour_data?: ThicknessContourPoint[];
  };
  // New layup structure matching actual JSON
  layup?: {
    total_layers: number;
  ...
  // Legacy winding_pattern for backwards compatibility
  winding_pattern?: {
    layers: Array<{
      type: 'helical' | 'hoop';
      angle_deg: number;
