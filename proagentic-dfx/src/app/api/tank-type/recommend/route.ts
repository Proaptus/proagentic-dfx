...
// Tank type for recommendation
type TankType = 'I' | 'II' | 'III' | 'IV' | 'V';

// Request type supporting both old and new field names
interface TankTypeRequest {
  requirements: {
    pressure_bar?: number;
...
function calculateTankTypeRecommendation(
  ...
  // Type selection logic based on weight target
  ...
  if (environment === 'marine' && recommendedType === 'V') {
    recommendedType = 'IV';
    confidence = 0.90;
    rationale = `Weight target suggests Type V but marine environment requires Type IV for proven certification path.`;
  }
