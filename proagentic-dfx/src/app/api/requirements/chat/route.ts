...
// Mock dialogue state tracking (reserved for future stateful conversations)
const _dialogueStates = new Map<string, {
  stage: number;
...
function generateIntelligentResponse(
  ...
  if (stage >= 2 && !extractedReqs.operating_temp_min_c) {
    return {
      ...
      extracted_requirements: allRequirements,
      suggestions: ['-40C to +85C (automotive)', '-55C to +70C (aviation)', '-10C to +50C (indoor)'],
      is_complete: false,
    };
  }
