...
export async function POST(request: NextRequest) {
  try {
    ...
    const requirements = extractRequirementsFromConversation(
...
function extractRequirementsFromConversation(
  ...
  if (
    allUserMessages.includes('automotive') ||
    allUserMessages.includes('car') ||
    allUserMessages.includes('vehicle')
  ) {
    ...
    allUserMessages.includes('aviation') ||
    allUserMessages.includes('aircraft') ||
    allUserMessages.includes('drone')
  ) {
    ...
    allUserMessages.includes('stationary') ||
    allUserMessages.includes('storage') ||
    allUserMessages.includes('facility')
  ) {
  ...
  // Capacity detection
  const capacityMatch = allUserMessages.match(
    /(\d+\.?\d*)\s*(kg|liters?|l)\s*(h2|hydrogen)?/i
  );
  if (capacityMatch) {
    ...
    if (capacityMatch[2].toLowerCase().startsWith('kg')) {
      ...
      requirements.internal_volume_liters = {
  ...
  if (weightMatch) {
    requirements.target_weight_kg = {
      value: parseInt(weightMatch[1], 10),
      confidence: 0.9,
    };
  }

  // Cost target detection
  const costMatch = allUserMessages.match(
    /(?:cost|budget|price).*?[€$]?\s*(\d+[,\d]*)/i
  );
  if (costMatch) {
  ...
  const tempMatch = allUserMessages.match(
    /(-?\d+)\s*°?c\s*to\s*\+?(-?\d+)\s*°?c/i
  );
  if (tempMatch) {
    requirements.operating_temp_min_c = {
      value: parseInt(tempMatch[1], 10),
      confidence: 0.95,
    };
    requirements.operating_temp_max_c = {
      value: parseInt(tempMatch[2], 10),
      confidence: 0.95,
    };
  }
  ...
  if (allUserMessages.includes('iso') || allUserMessages.includes('11119')) {
    ...
    allUserMessages.includes('un r134') ||
    allUserMessages.includes('r134')
  ) {
    requirements.certification_region = { value: 'EU', confidence: 0.95 };
  } else if (
    allUserMessages.includes('asme') ||
    allUserMessages.includes('usa') ||
    allUserMessages.includes('us market')
  ) {
...
function generateIntelligentResponse(
  ...
  if (stage === 0) {
    if (lowerMessage.includes('automotive')) {
      return {
        message:
          "Great! For automotive applications, I'll help you design a hydrogen fuel cell tank. The automotive industry typically uses 700 bar Type IV tanks for optimal gravimetric efficiency.\n\nNow, let's talk about capacity. How much hydrogen do you need to store? You can specify this as either:\n- Storage capacity in kg of H2 (typical: 5-6 kg for 500-600 km range)\n- Internal volume in liters (typical: 150-180 L)",
        extracted_requirements: allRequirements,
        suggestions: ['5 kg H2', '150 liters', '6 kg for 600 km range'],
        is_complete: false,
      };
    } else if (lowerMessage.includes('aviation')) {
      return {
        message:
          "Aviation applications - excellent choice! For aircraft and drones, weight is critical. Aviation typically uses 350 bar Type III or Type IV tanks to balance weight and volumetric efficiency.\n\nWhat's your target hydrogen storage capacity? Please specify in kg of H2.",
        extracted_requirements: allRequirements,
        suggestions: ['2 kg H2', '10 kg H2', '50 kg H2'],
        is_complete: false,
      };
    } else if (lowerMessage.includes('stationary')) {
      return {
        message:
          'Stationary storage - perfect for renewable energy integration! For stationary applications, we typically use 500 bar tanks where cost is often more important than weight.\n\nWhat storage capacity do you need? You can specify in kg of H2 or volume in liters.',
        extracted_requirements: allRequirements,
        suggestions: ['100 kg H2', '500 liters', '1000 liters'],
        is_complete: false,
      };
    }
  }
  ...
  if (stage >= 2 && !extractedReqs.operating_temp_min_c) {
    return {
      message:
        "Good! Let's define the operating environment.\n\nWhat temperature range will the tank experience?\n- Automotive standard: -40C to +85C\n- Aviation: -55C to +70C\n- Indoor stationary: -10C to +50C\n\nPlease specify your temperature range.",
      extracted_requirements: allRequirements,
      suggestions: [
        '-40C to +85C (automotive)',
        '-55C to +70C (aviation)',
        '-10C to +50C (indoor)',
      ],
      is_complete: false,
  ...
  return {
    message:
      'Thanks for that information! I\'m processing your requirements. Could you provide more details about your tank specifications?',
    extracted_requirements: allRequirements,
