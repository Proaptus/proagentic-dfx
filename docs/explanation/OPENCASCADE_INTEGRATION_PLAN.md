...
title: "OpenCascade.js CAD Integration Plan"
version: 1.0.0
date: 2025-12-09
owner: "@h2-tank-team"
status: accepted
...
**What it does**:
- âœ… Isotensoid dome profile calculation (good math)
- âœ… Layer-by-layer visualization
- âœ… Boss geometry positioning
- âœ… Fiber winding path visualization

**Critical Problems**:
- âŒ Uses `THREE.LatheGeometry` - simple mesh primitives, NOT CAD solids
- âŒ No B-rep topology - just triangulated meshes
- âŒ Cannot export to STEP format (required for engineering workflows)
- âŒ No parametric constraints or design intent
- âŒ No proper solid boolean operations
...
**File**: `src/lib/cad/opencascade-loader.ts` âœ… DONE
...
**File**: `src/lib/cad/tank-geometry.ts` âœ… DONE (skeleton)
...
|-------------|--------|----------------|
| **REQ-211**: Parametric tank solids | ðŸŸ¡ In Progress | `createTankLinerSolid()` |
| **REQ-212**: Isotensoid dome B-rep | ðŸŸ¡ In Progress | `createIsotensoidDome()` |
| **REQ-213**: Composite layer solids | ðŸŸ¡ Planned | `createCompositeLayer()` |
| **REQ-214**: Boss geometry | ðŸŸ¡ In Progress | `createBossGeometry()` |
| **REQ-215**: STEP export | ðŸŸ¡ In Progress | `exportToSTEP()` |
...
### 2. âŒ Mesh Extraction Complexity
**Issue**: `TopoDS_Shape` â†’ Triangle mesh requires OCC API knowledge
...
### 3. âŒ Virtual Filesystem for STEP Export
...
**Pros**:
- âœ… Cleaner API (wrapper around OCC)
- âœ… MIT license
- âœ… Active maintenance (updated 1 week ago)
- âœ… Better TypeScript support
- âœ… Smaller bundle (31 MB vs 67 MB)

**Cons**:
- âŒ Another abstraction layer
- âŒ Less direct control over OCC
- âŒ Fewer examples
...
**Status**: ðŸŸ¡ **Foundation Ready, Implementation In Progress**

**Files Created**:
- âœ… `src/lib/cad/opencascade-loader.ts` - WASM loader with TypeScript types
- âœ… `src/lib/cad/tank-geometry.ts` - B-rep geometry functions (skeleton)
- âœ… `OPENCASCADE_INTEGRATION_PLAN.md` - This document
...
**Risk Level**: ðŸŸ¡ Medium (WASM complexity, mesh extraction, but proven technology)
