...
export function CompareScreen() {
  const { selectedDesigns, setCurrentDesign, setScreen } = useAppStore();

  const [comparison, setComparison] = useState<CompareResponse | null>(null);
  const [loading, setLoading] = useState(false);

  // Default to A, B, C if none selected
  const designsToCompare = selectedDesigns.length >= 2 ? selectedDesigns : ['A', 'B', 'C'];
